function x = ct_low_mem_os_lalm_uss_c(...
    y,A,W,R,x0,...
    nIter,nBlock,eta,...
    xref,iROI,...
    iSave,sDir...
...
fprintf('Compute the uniform majorizer of the regularizer: L2...\n');
L2 = max(R.denom(R,zeros(np,1)));
...
% H = eta * Lmax I => L2 > max(eig(Lmax C'C))
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using [uniform step sizes] low-memory OS-LALM with H-weighting and continuation (nBlock = %g, eta = %g)...\\n',nBlock,eta);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iOrder
        s = rho*grad+(1-rho)*g;
        sigma = eta*(vb-et);
        x = proj(x-(s+sigma)./(rho*dwls+eta*L2));
