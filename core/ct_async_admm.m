  function [z, info] = ct_async_admm(x0, Ab, yi, R, varargin)
%|function [z, info] = ct_async_admm(x0, Ab, yi, R, [options])
%|
%| model-based 3d x-ray ct image reconstruction using alternating direction
%| method of multipliers with asynchronous update and nesterov's momentum
%| technique (see: iutzeler:13:ado)
%|
...
%|    order                   update order (default: 'fft')
...
z = tov(x0); z = max(z,voxmin); z = min(z,voxmax);
x = cell(1,nblock); xt = cell(1,nblock); d = cell(1,nblock);
for ib = 1:nblock, x{ib} = z; xt{ib} = z; d{ib} = zeros(np,1); end

info0 = arg.userfun(z,arg.userarg{:});
info = zeros(niter,1);

str.info = sprintf('Start solving X-ray CT image reconstruction problem using asynchronous ADMM (nblock = %g)...\\n',nblock);
fprintf(str.info);
str.log = str.info;
...
for iter = 1:niter
    ...
    for iset = 1:nblock
        ...
        xb = (1-theta(k))*xt{ib}+theta(k)*x{ib};
        grd = Ab{ib}'*(col(wi(:,:,ia)).*(Ab{ib}*xb-col(yi(:,:,ia))))+R.cgrad(R,xb)/nblock;
        den = denom/nblock+R.denom(R,xb)/nblock;
        x{ib} = (theta(k)*den.*x{ib}-grd+rho(k)*(z+d{ib}))./(theta(k)*den+rho(k));
        xt{ib} = (1-theta(k))*xt{ib}+theta(k)*x{ib};
        
        rz = alpha(k)*x{ib}+(1-alpha(k))*z;
        z = rz-d{ib};
        z = max(z,voxmin);
        z = min(z,voxmax);
    ...
    tt = toc;
    info(iter) = arg.userfun(z,arg.userarg{:});
    fprintf(' ');
    ...
    if any(arg.isave==iter)
        fld_write([arg.path 'x_iter_' num2str(iter) '.fld' ],tom(z));
    end
end
