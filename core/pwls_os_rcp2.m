  function [xs, info] = pwls_os_rcp2(x, Ab, yi, R, varargin)
%|function [xs, info] = pwls_os_rcp2(x, Ab, yi, R, [options])
%|
%| penalized weighted least squares estimation / image reconstruction
%| using relaxed first-order primal-dual (chambolle-pock) algorithm with
...
%|    gamma                   nes88 acceleration parameter (default 1)
%|
%| out
...
if isnumeric(arg.gamma)
    if arg.gamma>=0
        gamma = str2func(['@(k)' num2str(arg.gamma)]);
    else
        fail 'illegal nes88 acceleration parameter gamma?!';
    end
else
    gamma = str2func(arg.gamma);
end
...
for iter = 1:arg.niter
    ...
    for iset = 1:nblock
        ...
        num = g+pgrad;
        den = rho(k)*denom/relax+Rdenom*gamma(k)/(2-alpha(k));
        ...
        xb = 2*xh-x;
        x = alpha(k)*xh+(1-alpha(k))*x;
        
        xa = (1-gamma(k))*xa+gamma(k)*x;
