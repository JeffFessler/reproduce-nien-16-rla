function [x,rmsd] = ct_os_lalm_dmro(...
    y,A,W,R,x0,...
    nIter,nBlock,alpha,rho,dwls,rank1,eta,option,...
    xref,iROI,iFBP,...
    iSave,sDir...
    )
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-LALM-DMRO (nBlock = %g, alpha = %g, rho = %g, rank1 = %g, eta = %g, option = %s)...\\n',nBlock,alpha,rho,rank1,eta,option);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iOrder
        ...
        xbold = xb;
        xb = xb-inv_d_uv(sb+R.cgrad(R,xt)+tb,d0,ub,-ub);
