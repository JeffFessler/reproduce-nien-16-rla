function x = ct_low_mem_os_lalm_cc1(...
    y,A,W,R,x0,...
    nIter,nBlock,...
    xref,iROI,...
    iSave,sDir...
...
cgrad = R.cgrad(R,x);
wt = (rho*0+cgrad)/(rho+1);
et = (0-cgrad)/(rho+1);

str.info = sprintf('Start solving X-ray CT image reconstruction problem using low-memory OS-LALM with H-weighting and continuation [data + reg] (nBlock = %g)...\\n',nBlock);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iOrder
        s = rho*grad+(1-rho)*g;
        sigma = rho*(wt-et);
        x = proj(x-(s+sigma)./(rho*dsqs));
        grad = gradi{iblock}(x,y)*nBlock;
        g = (rho*grad+g)/(rho+1);
        cgrad = R.cgrad(R,x);
        wt = (rho*et+cgrad)/(rho+1);
        et = (et-cgrad)/(rho+1);
