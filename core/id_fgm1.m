  function [x, info] = id_fgm1(x, y, R, varargin)
%|function [x, info] = id_fgm1(x, y, R, [options])
%|
%| solve a edge-preserving image denoising problem:
%| 
%| minimize 1/2 (y-x)'(y-x) + R(x)
%|    x
%|
...
for iter = 1:niter
    ...
    % update info
    info(iter) = arg.userfun(tom(x),arg.userarg{:});
