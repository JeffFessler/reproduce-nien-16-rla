function x = ct_high_mem_gcd_os_lalm_c_nrs(...
    y,A,W,R,x0,...
    nIter,nBlock,eta,...
    xref,iROI,...
    iSave,sDir...
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using high-memory GCD OS-LALM with continuation [no restart] (nBlock = %g, eta = %g)...\\n',nBlock,eta);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iOrder
        ...
        Cx{di} = Ci{di}(x);
        % v{di} = prox(Cx{di}-e{di},thd{di});
        soft_shrink_ip1(v{di},Cx{di}-e{di},thd{di},R.nthread);
        e{di} = e{di}-Cx{di}+v{di};
		signew = eta*Cit{di}(Cx{di}-v{di}-e{di});
