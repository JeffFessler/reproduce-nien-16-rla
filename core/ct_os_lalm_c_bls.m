function [x,rmsd] = ct_os_lalm_c_bls(...
    y,A,W,R,x0,...
    nIter,nBlock,alpha,dwls,beta,option,...
    xref,iROI,iFBP,...
    iSave,sDir...
    )
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-LALM-c-BLS [backtracking line-search] (nBlock = %g, alpha = %g, beta = %g, option = %s)...\\n',nBlock,alpha,beta,option);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
