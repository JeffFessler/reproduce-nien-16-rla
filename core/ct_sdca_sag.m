  function [x, info] = ct_sdca_sag(x0, Ab, yi, R, varargin)
%|function [x, info] = ct_sdca_sag(x0, Ab, yi, R, [options])
%|
...
Ri = Reg1block(R,nreg);
gxi = cell(1,nreg); sgx = zeros(np,1,'single');
for ii = 1:nreg
    gxi{ii} = Ri{ii}.cgrad(Ri{ii},x);
    sgx = sgx+gxi{ii};
end
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using SDCA-SAG...\\n',nblock);
fprintf(str.info);
str.log = str.info;
...
for iter = 1:niter
    ...
    for iset = 1:nblock
        ...
        if rand<=psag
            ireg = randi(nreg);
            gxx = Ri{ireg}.cgrad(Ri{ireg},x);
            sgx = sgx-gxi{ireg}+gxx; gxi{ireg} = gxx;
            b = x-sgx./dreg;
        end
