...
x = x0;
g = zeros(np,1);
gtmp = gradi{iOrder(end)}(x,y)*nBlock;
g = (samp/nBlock).*gtmp+(1-samp/nBlock).*g;
u = g;

k = 1;
rho = a(k);

str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-LALM with continuation and SAG (nBlock = %g, nDenoise = %g)...\\n',nBlock,nDenoise);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
