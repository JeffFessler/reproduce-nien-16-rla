...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-Nes88g (nBlock = %g, nDenoise = %g)...\\n',nBlock,nDenoise);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iOrder
        ...
        gu = theta*gx+(1-theta)*gz;
        v = x-gu./(dwls+eps)/theta;
        x = denoise_box(x,(dwls+eps)*theta,v,R,dwls==0,x0,nDenoise);
		gx = gradi{iblock}(x,y)*nBlock;
        gz = theta*gx+(1-theta)*gz;
