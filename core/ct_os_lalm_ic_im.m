...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-LALM with intuitive downward continuation in the image form (nBlock = %g, nDenoise = %g)...\\n',nBlock,nDenoise);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iOrder
        v = x-gradi{iblock}(x+(1/rho-1)*z,y)*nBlock./(dwls+eps);
