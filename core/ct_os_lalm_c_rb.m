function x = ct_os_lalm_c_rb(...
    y,A,W,R,x0,...
    nIter,nBlock,nDenoise,dDilate,...
    xref,iROI,...
    iSave,sDir...
    )
...
iFixed = convn(dwls==0,ones(dDilate,dDilate,dDilate),'same')>0;
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-LALM with continuation and relaxed box constraint (nBlock = %g, nDenoise = %g)...\\n',nBlock,nDenoise);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
