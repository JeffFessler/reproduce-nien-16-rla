...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using [huber curv.] low-memory OS-LALM with H-weighting (nBlock = %g, eta = %g)...\\n',nBlock,eta);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iOrder
        s = rho*grad+(1-rho)*g;
        sigma = eta*(vb-et);
        x = proj(x-(s+sigma)./(rho*dwls+eta*R.denom(R,x)));
