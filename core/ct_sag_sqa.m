function x = ct_sag_sqa(...
    y,A,W,R,x0,...
    nIter,nBlock,nSQA,option,order,...
    xref,iROI,...
    iSave,sDir...
    )
...
[forwi,backi,iVals,gi,dwls] = setup_ct_sag(A,W,nBlock,nIter,order);
...
switch option
    ...
    otherwise
        exit('quad. approx. is not available!?');
end
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using SAG (nBlock = %g, nSQA = %g, option = %s, order = %s)...\\n',nBlock,nSQA,option,order);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iVals((1:nBlock)+nBlock*(iter-1))
        ...
        gnew = forwi{iblock}(x,y);
        d = d-backi{iblock}(gi{iblock}-gnew);
        gi{iblock} = gnew;
        x = proj((1-dreg./denom).*x-(d/m-dreg.*b)./denom);
        
        if mod(k,floor(nBlock/nSQA))==0
