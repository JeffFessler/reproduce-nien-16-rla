function [x,rmsd,ttoc] = ct_os_dmro_oracle(...
    y,A,W,R,x0,...
    nIter,nBlock,dwls,nFISTA,option,...
    xref,iROI,iFBP,...
    iSave,sDir...
    )
...
switch option
    case {'max-curv','huber'}
        den = dwls+R.denom(R,zeros(np,1,'single'))+eps;
    otherwise
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-DMRO-oracle (nBlock = %g, nFISTA = %g, option = %s)...\\n',nBlock,nFISTA,option);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for ib = 1:nBlock
        ...
        numl = nBlock*(Ai{iblock}'*(col(W.arg.diag(:,:,ia)).*(Ai{iblock}*x-col(y(:,:,ia)))));
        numr = R.cgrad(R,x);
        if strcmp(option,'huber')
            den = dwls+R.denom(R,x)+eps;
        end
        curv = dwls.*dx-dg;
        u = div0(curv,dx'*curv);
        
        num = numl+numr-den.*x+(u'*x)*u;
        xold = x;
        z = x; v = z; zold = z; told = 1;
        for fiter = 1:nFISTA
            z = proj(v-(num+den.*v-(u'*v)*u)./den);
            if (v-z)'*(z-zold)>0
                t = 1;
                v = z;
            else
        ...
        dx = x-xold;
        dg = nBlock*(Ai{iblock}'*(col(W.arg.diag(:,:,ia)).*(Ai{iblock}*dx)));
