function [x,rmsd,ttoc] = ct_os_sag_lalm_c_oracle1(...
    y,A,W,R,x0,...
    nIter,nBlock,dwls,nReg,nSAG,option,...
    xref,iROI,iFBP,...
    iSave,sDir...
    )
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-SAG-LALM-c-oracle1 (nBlock = %g, nReg = %g, nSAG = %g, option = %s)...\\n',nBlock,nReg,nSAG,option);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for ib = 1:nBlock
        ...
        v = x;
        for iSAG = 1:nSAG
            num = rho*dwls.*(v-x)+s+sgz;
            den = rho*dwls+sdz;
            v = proj(v-num./den);
            
            ir = randi(nReg);
            gzz = Ri{ir}.cgrad(Ri{ir},v);
            sgz = sgz-gzi{ir}+gzz; gzi{ir} = gzz;
            if strcmp(option,'huber')
                dzz = Ri{ir}.denom(Ri{ir},v);
