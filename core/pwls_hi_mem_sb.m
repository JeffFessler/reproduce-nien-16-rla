  function [xs, info] = pwls_hi_mem_sb(x, A, yi, R, varargin)
%|function [xs, info] = pwls_hi_mem_sb(x, A, yi, R, [options])
%|
%| penalized weighted least squares estimation / image reconstruction
%| using high-memory split bregman method.
%|
...
thd = cell2mat(thd);
G = A'*(Gdiag(wi,'class','Fatrix')*A)+eta*(C'*C);
b = A'*(wi(:).*yi(:));
