function x = ct_os_partial_dpm_sqs(...
    y,A,W,R,x0,...
    nIter,nBlock,d,...
    xref,iROI,...
    iSave,sDir...
...
dwls = compute_diag_majorizer(Ab,W,d);
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-dpm-SQS (nBlock = %g)...\\n',nBlock);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
