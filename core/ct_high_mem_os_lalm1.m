function x = ct_high_mem_os_lalm1(...
    y,A,W,R,x0,...
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using high-memory OS-LALM (nBlock = %g, rho = %g, eta = %g)...\\n',nBlock,rho,eta);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iOrder
        x = proj(x-(rho*grad+(1-rho)*g+eta*C'*p)./(rho*dwls+eta*(4*R.M)));
        grad = gradi{iblock}(x,y)*nBlock;
        Cx = C*x;
        g = (rho*grad+g)/(rho+1);
        perform_updates_l1(v,e,p,Cx,thd,R.nthread);
