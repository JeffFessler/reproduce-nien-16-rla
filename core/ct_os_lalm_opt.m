...
% rho = norm(sqrt(W.arg.diag(:)).*(A*(x0-xref)))/norm(sqrt(dwls+eps).*(x0-xref));
dx = x0-xref;
Adx = A*dx;
rho = sqrt(Adx'*(W*Adx))/sqrt(dx'*((dwls+eps).*dx));

x = x0;
g = gradi{iOrder(end)}(x,y)*nBlock;
u = g;

str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-LALM with optimal rho = %g (nBlock = %g, nDenoise = %g)...\\n',rho,nBlock,nDenoise);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
