function x = ct_os_nes05_partial_bb(...
    y,A,W,R,x0,...
    nIter,nBlock,...
    xref,iROI,...
    iSave,sDir...
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-Nes05-partialBB (nBlock = %g)...\\n',nBlock);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iOrder
		gradz = gradi{iblock}(z,y)*nBlock;
        grad = gradz+R.cgrad(R,z);
        sgrad = sgrad+t*grad;
		...
		if iter==1 && iblock==iOrder(1)
		    ...
		    dg = gradz-gold;
			dz = z-zold;
			sdwls = (min((dz'*dg)/(dz'*(dsqs.*dz)),1)*iROI+(1-iROI)).*dwls;
		end
		gold = gradz;
		zold = z;
		dsqs = sdwls+R.denom(R,z)+eps;
