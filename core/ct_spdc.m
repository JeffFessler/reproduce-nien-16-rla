  function [x, info] = ct_spdc(x0, Ab, yi, R, varargin)
%|function [x, info] = ct_spdc(x0, Ab, yi, R, [options])
%|
%| model-based 3d x-ray ct image reconstruction using stochastic primal-
%| dual coordinate method with K = sqrt(W)A (see zhang:14:spd)
%|
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using SPDC (nblock = %g)...\\n',nblock);
fprintf(str.info);
str.log = str.info;
