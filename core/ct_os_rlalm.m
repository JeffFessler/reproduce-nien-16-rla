  function [x, info] = ct_os_rlalm(x0, Ab, yi, R, varargin)
%|function [x, info] = ct_os_rlalm(x0, Ab, yi, R, [options])
%|
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using relaxed OS-LALM (nblock = %g)...\\n',nblock);
fprintf(str.info);
str.log = str.info;
...
for iter = 1:niter
    ...
    for iset = 1:nblock
        ...
        num = rho(k)*denom0.*x+(1-rho(k))*g-rho(k)*h+R.cgrad(R,x);
        switch reg_type
            case 'huber'
