  function [x, info] = ct_os_rcp(x0, Ab, yi, R, varargin)
%|function [x, info] = ct_os_rcp(x0, Ab, yi, R, [options])
%|
%| model-based 3d x-ray ct image reconstruction using relaxed chambolle-
%| pock algorithm with (optionally relaxed) ordered subsets
%|
...
%|    corr                    correction of step size (default: 0)
%|
%| out
...
if isnumeric(arg.corr)
    if(arg.corr==0)
        gain = @(k) 1;
    else
        gain = @(k) 1/(2-min(alpha(k),2-1e-5));
    end
else
    fail 'illegal option of step size correction?!';
end
...
ib = ios(end);
ia = ib:nblock:na;
grad = nblock*(Ab{ib}'*(col(wi(:,:,ia)).*(Ab{ib}*xb-col(yi(:,:,ia)))));
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-rCP (nblock = %g)...\\n',nblock);
fprintf(str.info);
str.log = str.info;
...
for iter = 1:niter
    ...
    for iset = 1:nblock
        ...
        num = g+R.cgrad(R,x);
        den = rho(k)*denom/relax+gain(k)*R.denom(R,x);
        ...
        gh = (rho(k)*grad+g)/(rho(k)+1);
        
        x = (1-alpha(k))*x+alpha(k)*xh;
        g = (1-alpha(k))*g+alpha(k)*gh;
        
        fprintf([back count],iset);
    end
	
    tt = toc;
    info(iter) = arg.userfun(xh,arg.userarg{:});
    fprintf(' ');
    ...
    if any(arg.isave==iter)
        fld_write([arg.path 'x_iter_' num2str(iter) '.fld' ],tom(xh));
    end
end
