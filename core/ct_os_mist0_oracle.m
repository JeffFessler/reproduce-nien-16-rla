function [x,rmsd] = ct_os_mist0_oracle(...
    y,A,W,R,x0,...
...
switch option
    case {'max-curv','huber'}
        denom = dwls+R.denom(R,zeros(np,1,'single'));
    otherwise
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-MIST-oracle (nBlock = %g, eta = %g, option = %s)...\\n',nBlock,eta,option);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iOrder
        if strcmp(option,'huber')
            denom = dwls+R.denom(R,x);
        end
        des = x-div0(grad,denom); % gradient descent
        ggx = proj(des)-x; % generalized gradient
        delta = x-xold;
        gamma = denom.*delta-(grad-gold);
        alpha = 2*eta*div0(gamma'*ggx,gamma'*delta);
        
        xold = x;
        x = proj(des+alpha*div0(gamma,denom));
