function x = ct_high_mem_admm(...
    y,A,W,R,x0,...
    nIter,nPCG,pType,rho,eta,...
    xref,iROI,...
    iSave,sDir...
    )
...
switch pType
    ...
    case 'slice-circ'
        P = slice_circ_precon(A,rho,sqrt(eta)*C,A.imask);
    otherwise
        P = 1;
end
tt = toc;
...
Ax = A*x;
u = (Wy+rho*Ax)./(w+rho); d = -Ax+u; % d = 0 initially

Cx = C*x;
v = prox(Cx); e = -Cx+v; % e = 0 initially

str.info = sprintf('Start solving X-ray CT image reconstruction problem using high-memory ADMM (nPCG = %g, pType = %s, rho = %g, eta = %g)...\\n',nPCG,pType,rho,eta);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
	
    x = qpwls_pcg2_no_warning(x,B,rho*A'*(u+d)+eta*C'*(v+e),0,'precon',P,'niter',nPCG,'isave','last');
    Ax = A*x;
    u = (Wy+rho*(Ax-d))./(w+rho);
    d = d-Ax+u;
