...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-SQS (nBlock = %g, thd = %g, cst = %g)...\\n',nBlock,thd,cst);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iOrder
        grad = gradi{iblock}(x,y)+R.cgrad(R,x);
		
		x = max(x-grad./dsqs,0); x(iFixed) = x0(iFixed);
