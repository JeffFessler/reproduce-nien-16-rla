function x = r_ct_os_lalm_c(...
    y,A,W,R,x0,...
    nIter,nBlock,nDenoise,thd,cst,lb,...
    xref,iROI,...
    iSave,sDir...
    )
...
k = 1;
rho = rho_sequence(k,lb);

str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-LALM with continuation (nBlock = %g, nDenoise = %g, thd = %g, cst = %g, lb = %g)...\\n',nBlock,nDenoise,thd,cst,lb);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
