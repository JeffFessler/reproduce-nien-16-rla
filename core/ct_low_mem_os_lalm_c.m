function x = ct_low_mem_os_lalm_c(...
    y,A,W,R,x0,...
    nIter,nBlock,eta,...
    xref,iROI,...
    iSave,sDir...
...
rho_min = 0;
a = @(k) single(max((pi/k*sqrt(1-(pi/2/k)^2))*(k>1)+(k<=1),rho_min));
...
% H = eta * diag{R.wt} * diag{wpot(Cx(k))}
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using [huber curv.] low-memory OS-LALM with H-weighting and continuation (nBlock = %g, eta = %g)...\\n',nBlock,eta);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iOrder
        s = rho*grad+(1-rho)*g;
        sigma = eta*(vb-et);
        x = proj(x-(s+sigma)./(rho*dwls+eta*R.denom(R,x)+eps));
