function x = ct_sag_nes83(...
    y,A,W,R,x0,...
    nIter,nBlock,order,option,...
    xref,iROI,...
    iSave,sDir...
    )
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-Nes83 with max curvature (nBlock = %g, order = %s, option = %s)...\\n',nBlock,order,option);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
