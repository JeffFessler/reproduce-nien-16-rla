...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-LALM with continuation in the image form [z] (nBlock = %g, nDenoise = %g)...\\n',nBlock,nDenoise);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iOrder
        u = rho*x+(1-rho)*z;
        v = x-(gradi{iblock}(u,y)*nBlock)./(dwls+eps)/rho;
    ...
    if sum(iSave==iter)>0
        fld_write([sDir 'x_iter_' num2str(iter) '.fld' ],vecx2matx(z));
    end
end
