function x = ct_high_mem_os_lalm_c(...
    y,A,W,R,x0,...
    nIter,nBlock,eta,...
    xref,iROI,...
    iSave,sDir...
...
Cx = C*x;
% v = Cx; e = 0*Cx;
v = prox(Cx,thd); e = -Cx+v; % e = 0 initially

k = 1;
rho = a(k);

str.info = sprintf('Start solving X-ray CT image reconstruction problem using high-memory OS-LALM with continuation (nBlock = %g, eta = %g)...\\n',nBlock,eta);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
