  function [x, info] = ct_os_gsm(x0, Ab, yi, R, varargin)
%|function [x, info] = ct_os_gsm(x0, Ab, yi, R, [options])
%|
%| model-based 3d x-ray ct image reconstruction using gradient sliding
%| method [lan:14:gsf] with (optionally relaxed) ordered subsets
%|
...
ib = ios(nblock);
ia = ib:nblock:na;
zeta = nblock*(Ab{ib}'*(col(wi(:,:,ia)).*(Ab{ib}*x-col(yi(:,:,ia)))));
g = zeta;
t = 1;
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-GSM (nblock = %g)...\\n',nblock);
fprintf(str.info);
str.log = str.info;
...
for iter = 1:niter
    ...
    for iset = 1:nblock
        ...
        % update s
        s = (1-theta)*g+theta*zeta;
        % update x
        num = s+R.cgrad(R,x);
        den = theta*denom+R.denom(R,x);
        x = x-relax*num./den;
        x = max(x,voxmin);
        x = min(x,voxmax);
        % update zeta
