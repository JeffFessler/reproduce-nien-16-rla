...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-Nes05-BB (nBlock = %g)...\\n',nBlock);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iOrder
		...
		if iter==1 && iblock==iOrder(1)
		    ...
		    dg = grad-gold;
			dz = z-zold;
			dsqs = (min((dz'*dg)/(dz'*(dsqs.*dz)),1)*iROI+(1-iROI)).*dsqs;
			gold = grad;
			zold = z;
		end
