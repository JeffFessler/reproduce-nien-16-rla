  function [x, info] = ct_pspdc(x0, Ab, yi, R, varargin)
%|function [x, info] = ct_pspdc(x0, Ab, yi, R, [options])
%|
%| model-based 3d x-ray ct image reconstruction using preconditioned
%| stochastic primal-dual coordinate method with K = sqrt(M)A (see zhang:14:spd)
%|
...
%|    mi       [ns nt na]     precondition matrix (default: [] for uniform)
%|
%| out
...
if ndims(mi)~=3 || (size(mi,3)==1 && nblock>1)
    fail 'bad mi size';
end
pi = div0(wi,mi);
...
if ~isnumeric(denom)
    if strcmp(denom,'max')
        ...
        for ib = 1:nblock
            ia = ib:nblock:na;
            denom = max(Ab{ib}'*(col(mi(:,:,ia)).*(Ab{ib}*ones(np,1))),denom);
        end
        denom = denom*nblock;
    else
        error denom;
...
xr = x; gr = R.cgrad(R,xr);

info0 = arg.userfun(x,arg.userarg{:});
info = zeros(niter,1);

str.info = sprintf('Start solving X-ray CT image reconstruction problem using preconditioned SPDC (nblock = %g)...\\n',nblock);
fprintf(str.info);
str.log = str.info;
...
for iter = 1:niter
    ...
    for iset = 1:nblock
        ...
        znew = (col(z(:,:,ia))+rho(k)*(col(wi(:,:,ia)).*(Ab{ib}*xb-col(yi(:,:,ia)))))./(col(pi(:,:,ia))+rho(k));
        dAz = Ab{ib}'*(znew-col(z(:,:,ia)));
        z(:,:,ia) = reshape(znew,size(z(:,:,ia)));
        
        % num = u+dAz*nblock+R.cgrad(R,x);
        % den = rho(k)*denom+R.denom(R,x);
        num = u+dAz*nblock+Rdenom.*(x-xr)+gr;
