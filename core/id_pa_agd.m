  function [x, info] = id_pa_agd(x, y, R, varargin)
%|function [x, info] = id_pa_agd(x, y, R, [options])
%|
...
niter = arg.niter;
K = arg.K;
mu = 1; % strongly convex parameter
...
for iter = 1:niter
    ...
    % x-update
    wp = w-eta*(w-y);
    % x = tom(edge_shrink(tov(wp),R,one/eta,'voxmax',box));
    % x = tom(edge_shrink1(tov(wp),R,eta,one,'voxmax',box,'K',K));
    x = tom(edge_shrink_mat(tov(wp),R,eta,one,'voxmax',box));
    % z-update
