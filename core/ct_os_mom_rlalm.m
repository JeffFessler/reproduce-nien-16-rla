  function [xt, info] = ct_os_mom_rlalm(x0, Ab, yi, R, varargin)
%|function [xt, info] = ct_os_mom_rlalm(x0, Ab, yi, R, [options])
%|
...
Rden0 = R.denom(R,zeros(np,1,'single'));
reg_type = arg.reg_type;

info0 = arg.userfun(x,arg.userarg{:});
info = zeros(niter,1);

str.info = sprintf('Start solving X-ray CT image reconstruction problem using relaxed OS-LALM with momentum (nblock = %g)...\\n',nblock);
fprintf(str.info);
str.log = str.info;
...
for iter = 1:niter
    ...
    for iset = 1:nblock
        ...
        % x-update
        xb = (1-tk)*xt+tk*x;
        num = rk*denom0.*x+(1-rk)*g-rk*h+R.cgrad(R,xb);
        switch reg_type
            case 'huber'
                den = rk*denom/relax+tk*R.denom(R,xb);
            case 'max'
                den = rk*denom/relax+tk*Rden0;
            otherwise
                fail 'bad reg_type!?';
        end
        % den = rk*denom/relax+tk*R.denom(R,xb);
