function [x,objv,rmsd] = ir_ilalm_c(x0,y,A,R,niter,xref)
...
fprintf('Start solving image restoration problem using inexact LALM with continuation and restart...\n');
for iter = 1:niter
    gold = g;
    
    v = rho*g+(1-rho)*u;
    x = x-(v+R.cgrad(R,x))/rho./(L+R.denom(R,x)/rho);
    g = grad(x);
    u = (rho*g+u)/(rho+1);
