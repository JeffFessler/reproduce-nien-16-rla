function [x,rmsd,ttoc] = ct_os_stoc_lalm_c_oracle(...
    y,A,W,R,x0,...
    nIter,nBlock,alpha,dwls,nReg,b,option,...
    xref,iROI,iFBP,...
    iSave,sDir...
    )
...
c = (b-1)/(sqrt(nBlock*nIter)-1); eta = @(l) c*sqrt(l)+(1-c);
% eta = @(l) 1;
% b = 2; c = (b-1)/(nBlock*nIter-1); eta = @(l) c*l+(1-c);

ttoc = zeros(nIter,1);

str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-Stoc-LALM-c-oracle (nBlock = %g, alpha = %g, nReg = %g, b = %g, option = %s)...\\n',nBlock,alpha,nReg,b,option);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
