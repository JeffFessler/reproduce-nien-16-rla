  function [x, info] = ct_os_rlalm0(x0, Ab, yi, R, varargin)
%|function [x, info] = ct_os_rlalm0(x0, Ab, yi, R, [options])
%|
%| model-based 3d x-ray ct image reconstruction using simple relaxed
%| linearized augmented lagrangian method with (optionally relaxed) ordered
%| subsets
%|
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using simple OS-rLALM (nblock = %g)...\\n',nblock);
fprintf(str.info);
str.log = str.info;
...
for iter = 1:niter
    ...
    for iset = 1:nblock
        ...
        num =(rho(k)*grad+(1-rho(k))*g)+R.cgrad(R,x);
