  function [x, info] = ct_os_lal(x0, Ab, yi, R, varargin)
%|function [x, info] = ct_os_lal(x0, Ab, yi, R, [options])
%|
...
%|    reg_type                regularization type (default: 'huber')
%|    lal_type                linearized al type (default: 'admm')
%|                            'admm' | 'alm'
%|
%| out
...
reg_type = arg.reg_type;
lal_type = arg.lal_type;
...
Rden = R.denom(R,zeros(np,1,'single'));

info0 = arg.userfun(x,arg.userarg{:});
info = zeros(niter,1);

str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-LALM (nblock = %g)...\\n',nblock);
fprintf(str.info);
str.log = str.info;
...
for iter = 1:niter
    ...
    for iset = 1:nblock
        ...
        switch lal_type
            case 'admm'
                sdir = rho(k)*grad+(1-rho(k))*g;
            case 'alm'
                sdir = (rho(k)*grad+g)/(rho(k)+1);
            otherwise
                fail 'bad lal_type';
        end
        
        num = sdir+R.cgrad(R,x);
        switch reg_type
            case 'huber'
                den = rho(k)*denom/relax+R.denom(R,x);
            case 'max'
                den = rho(k)*denom/relax+Rden;
            otherwise
                fail 'bad reg_type!?';
        end
