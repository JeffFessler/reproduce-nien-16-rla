function [x,rmsd] = ct_os_mist_oracle(...
    y,A,W,R,x0,...
...
g = nBlock*(Ai{iblock}'*(col(W.arg.diag(:,:,ia)).*(Ai{iblock}*x-col(y(:,:,ia)))));

dx = zeros(np,1,'single');
dg = zeros(np,1,'single');

switch option
    case {'max-curv','huber'}
        den = dwls+R.denom(R,zeros(np,1,'single'));
    otherwise
