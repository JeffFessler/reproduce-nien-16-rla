function x = ct_high_mem_gcd_os_lalm_c_gf1(...
    y,A,W,R,x0,...
    nIter,nBlock,eta,...
    xref,iROI,...
    iSave,sDir...
...
pot = R.pot{1};
param = single([pot.delta pot.param(1) pot.param(2)]);
fprintf('Compute weighting matrix in R...\n');
tic;
reg = cell(1,R.M);
for ii = 1:R.M
    reg{ii} = R.wt.col(ii)/eta;
    fprintf('*');
end
tt = toc;
...
for ii = 1:R.M
	...
	Cx{ii} = Ci{ii}(x);
	v{ii} = prox(Cx{ii},reg{ii});
	e{ii} = -Cx{ii}+v{ii};
    p{ii} = Cx{ii}-v{ii}-e{ii};
	sig{ii} = eta*Cit{ii}(p{ii});
	sigma = sigma+sig{ii};
	fprintf('*');
end
tt = toc;
...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using high-memory GCD OS-LALM with continuation (nBlock = %g, eta = %g)...\\n',nBlock,eta);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iOrder
        ...
        Cx{di} = Ci{di}(x);
        perform_updates_gf1(v{di},e{di},p{di},Cx{di},reg{di},param,R.nthread);
        signew = eta*Cit{di}(p{di});
