...
str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-Nes88 (nBlock = %g, nDenoise = %g)...\\n',nBlock,nDenoise);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iOrder
        ...
        u = theta*x+(1-theta)*z;
        v = x-(gradi{iblock}(u,y)*nBlock)./(dwls+eps)/theta;
