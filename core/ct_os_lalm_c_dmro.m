function [x,rmsd] = ct_os_lalm_c_dmro(...
    y,A,W,R,x0,...
    nIter,nBlock,alpha,dwls,rank1,eta,option,...
    xref,iROI,iFBP,...
    iSave,sDir...
    )
...
d0 = (rho*dwls+alpha*dreg+eps)*(1+eta);
ub = zeros(A.np,1);

rmsd = zeros(nIter+1,1);
rmsd(1) = rms(x-xref);

str.info = sprintf('Start solving X-ray CT image reconstruction problem using OS-LALM-DMRO (nBlock = %g, alpha = %g, rank1 = %g, eta = %g, option = %s)...\\n',nBlock,alpha,rank1,eta,option);
fprintf(str.info);
str.log = str.info;
for iter = 1:nIter
    tic;
    ...
    for iblock = iOrder
        ...
        xbold = xb;
        xb = xb-inv_d_uv(sb+R.cgrad(R,xt)+tb,d0,rho*ub,-ub);
        % -- compute gradient of xb
        gxbold =gxb;
        ia = iblock:nBlock:A.odim(end);
        gxb = nBlock*(Ai{iblock}'*(col(W.arg.diag(:,:,ia)).*(Ai{iblock}*xb-col(y(:,:,ia)))));
        % -- update rank-one part
        if rank1
            % tmp = dwls.*(xb-xbold)-(gxb-gxbold);
            tmp = dwls.*(xb-xbold)-nBlock*(Ai{iblock}'*(col(W.arg.diag(:,:,ia)).*(Ai{iblock}*(xb-xbold))));
            ub = tmp/sqrt((xb-xbold)'*tmp);
        end
